...
`Port`는 `Fake` 주입 ->`Java`의 `DI`, `patch`는 `time.sleep`·`yf.Ticker`처럼 **수정할 수 없는 외부 라이브러리에만** 썼다. `Java`에서 허용하는 "어쩔 수 없는 외부 라이브러리" 예외가 이 범위다. 도메인 객체를 patch로 우회한 곳은 없다.

격리/속도도 `Python`에서는 약하다. 모듈 속성 교체는 `JVM` 클래스 재정의와 달리 네임스페이스 한 칸을 바꿨다 되돌리는 수준이라 가볍고, `pytest-mock`의 `mocker` fixture가 테스트 종료 시 자동 복원하므로 `try-with-resources`를 손으로 닫다 깜빡할 여지가 없다. **같은 일을 `Java`는 비싸게(또는 `DI`로 우회해서) 하고, `Python`은 모듈 네임스페이스에서 싸게 한다.**
