...
클래스단에 붙인(`@Configuration` 바로 아래) `@Order(1)`은 config 빈 자신의 순서일 뿐, 그 안에서 생성된 advisor들에게 상속되지 않는다. `@Order(1)`은 문법적으로 보자면 에러가 아니였기에 컴파일러도 Spring도 아무 문제가 없었다. 그래서 **잘 돌아가는 것처럼 보였다.**
