...
boolean acquired = lock.tryLock(5, 3, TimeUnit.SECONDS);
//                              │  │
//                              │  └─ leaseTime: 락 유지 시간 (3초)
//                              └─ waitTime: 락 대기 시간 (최대 5초)
```

| 파라미터 | 값 | 의미 |
| :--- | :--- | :--- |
| `waitTime` | `5초` | 락을 못 잡으면 최대 `5초`까지 대기 |
| `leaseTime` | `3초` | 락 획득 후 `3초` 뒤 자동 해제 |
...
| 체크 | 이유 |
| :--- | :--- |
| `isHeldByCurrentThread()` | 본인이 잡은 락만 해제 가능 |
| `finally` 블록 | 예외 발생해도 반드시 락 해제 |
...
동기 처리의 한계를 확인했다. 다음에는 **Kafka를 이용한 비동기 처리**를 적용해본다.
