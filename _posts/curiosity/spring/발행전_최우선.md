...
그럼 순서를 못 받은 advisor는 몇 번이 되는가. `DefaultPointcutAdvisor`의 기본값은 `Ordered.LOWEST_PRECEDENCE`, int의 최댓값이다. **가장 안쪽 겹**으로 등록된다. 내 트랜잭션은 1등으로 지정된 게 아니라, 순번이 통째로 증발해서 꼴등으로 지정되어 있었다.
