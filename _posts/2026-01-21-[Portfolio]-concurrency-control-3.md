...
## Watchdog 을 이용한 락 자동 연장

### 기존 코드의 잠재적 문제
`leaseTime`을 고정값으로 설정하면, **처리가 예상보다 오래 걸릴 때 문제가 발생할 수 있다.**
...
`leaseTime`을 `3초로` 설정하면, 처리가 3초 이상 걸릴 때 **락이 자동 해제**된다. `Java`는 이를 모른 채 처리를 계속하고, 다음 요청이 락을 획득해버린다.
```
요청 1: [락 획득][────── 처리 4초 걸림 ──────]
                              │
                        3초 경과, 락 해제됨 (모름)
...
→ 두 요청이 동시에 DB 작업이 이루어질 수 있음
```

### Watchdog?
`Redisson`이 제공하는 **락 자동 연장 기능**이다. `leaseTime`을 `-1`로 설정하면 활성화된다.
```java
...
- `unlock()` 호출하면 즉시 해제
- 서버가 죽으면 `Watchdog`도 멈춤 → `30초` 후 자동 해제

### 언제 사용하나?

| 상황 | 권장 설정 |
| :--- | :--- |
| 처리 시간 예측 가능 | `leaseTime` 고정값 |
| 처리 시간 예측 불가 (외부 API 등) | **Watchdog (`-1`)** |

현재 프로젝트는 `DB` 작업만 수행하고 처리 시간이 일정하므로 `leaseTime=3`으로 충분하다.
실무에서 외부 `API` 호출 등 처리 시간이 가변적인 경우 `Watchdog` 사용을 권장한다.
...
| **평균 응답시간** | `9,040ms` | `7,310ms` | `4,160ms` |
| **최대 응답시간** | `13,610ms` | `9,590ms` | `7,040ms` |
| **총 소요시간** | `13.9s` | `10.3s` | `7.5s` |
| **Deadlock** | 발생 | 없음 | 없음 |
...
### 여전히 남은 문제
평균 `4.1초`는 많이 개선됐지만, **근본적인 한계가 있다.**

```
요청 500개 → [락 획득 대기] → [순차 처리] → 응답
             ^^^^^^^^^^^^^
             한 번에 하나만 처리
```
**줄 세우기**와 같은 락 기반 동시성 제어는 아무리 `Redis`가 빨라도 **뒤에 있는 요청은 기다릴 수밖에 없다.**

### 그런데 생각해보면
지금까지 사용한 `Redis`의 `SET NX`도 원자적 연산이었다. `Redis`는 이런 원자적 연산을 여러 개 제공한다.

| 명령어 | 동작 | 원자성 |
|:---|:---|:---|
| `SET NX` | 키 없으면 생성 | `true` |
| `INCR` | 값 1 증가 | `true` |
| `DECR` | 값 1 감소 | `true` |
...
> 결과가 `0 이상`이면 성공, `음수`면 실패로 처리하면 된다.

### 다음 단계: Phase 4 - Redis DECR + Kafka 후처리
```
요청 → Redis DECR (재고 판정) → 즉시 응답
              │
              └─ 성공 시 → Kafka → DB 저장/알림
```

**다룰 내용:**
- `Redis DECR`을 이용한 **락 없는 재고 관리**
- 즉시 응답 후 `Kafka`로 **후처리 분리**
- `DB` 저장 실패 시 **재시도 전략**
- 목표: 평균 응답시간 `4초` → `수십 ms`