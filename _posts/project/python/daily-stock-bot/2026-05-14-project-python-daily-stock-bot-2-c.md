...
## 이번 글에서 배운 것
1. **`Port` 규약이 같아도 어댑터 코드가 같지는 않다**. `SlackNotifier`/`DiscordNotifier`는 `Port`가 같지만 포맷 차이로 10% 정도만 차이가 존재한다. 
2. **fail-fast는 외부 의존성 검증에 적합하다**. `webhook_url`, `API_KEY` 누락은 어댑터 생성 시점에 잡아낸다.
3. **`main.py`는 로직이 아니라 조립이다**. 어댑터 조립, 단계별 예외 처리, exit code 정책. 비즈니스 로직은 어댑터와 도메인에 있다.
