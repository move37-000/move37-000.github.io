...
### `Java`의 `mockStatic`과 같은 것 아닌가
`Java` 진영에는 "`mockStatic` 같은 바이트코드 조작 mocking은 어쩔 수 없는 외부 라이브러리가 아니면 금지"라는 철학이 있다. 이유는 세 가지다.

- **격리가 깨진다.** `try-with-resources`로 닫는 걸 깜빡하면 패치가 다음 테스트로 샌다. "내 PC는 통과, `CI`만 실패"의 단골 원인이다.
- **무겁다.** `JVM`에 로드된 클래스를 테스트마다 바이트코드 수준에서 재정의하므로, 테스트가 수천 개가 되면 빌드 시간에 체감된다.
- **설계 개선을 막는다.** patch로 우회 가능하면 굳은 구조를 그대로 두게 된다. "테스트하기 어려운 코드는 설계가 잘못된 코드"라는 신념과 충돌한다.

이 철학은 이 프로젝트의 이원화와 충돌하지 않는다. 오히려 같은 결론이다. `Port`(우리 계약)는 `Fake` 주입 — `Java`의 `DI`에 해당 — 으로 풀었고, `patch`는 `time.sleep`·`yf.Ticker`처럼 **우리가 수정할 수 없는 외부 라이브러리에만** 썼다. `Java` 철학이 허용하는 "어쩔 수 없는 외부 라이브러리" 예외가 정확히 이 범위다. 우리 도메인 객체를 patch로 우회한 곳은 없다.

격리·속도 우려도 `Python`에서는 약하다. 모듈 속성 교체는 `JVM` 클래스 재정의와 달리 네임스페이스 한 칸을 바꿨다 되돌리는 수준이라 가볍고, `pytest-mock`의 `mocker` fixture가 테스트 종료 시 자동 복원하므로 `try-with-resources`를 손으로 닫다 깜빡할 여지가 없다. **같은 일을 `Java`는 비싸게(또는 `DI`로 우회해서) 하고, `Python`은 모듈 네임스페이스에서 싸게 한다.**
