...
### 동기
공공기관 보안 내부망 업무 특성상 동시성 제어 처리를 하지 않는다.(**애초에 트래픽이 적기도 하다...**) 하지만 다른 기업들은 동시성 제어 처리가 일상이다. 쿠폰 선착순 발급, 티켓 예매, 한정판 구매 등.. 어떻게 구현하고 어떻게 동작하는지 매번 궁금했다. 나도 저런 문제를 **직접 경험하고 해결**해보고 싶었다.

### 목표
최근 쿠팡에서 이벤트로 진행한 **선착순 쿠폰** 을 경험해보고, 그것을 목표로 삼았다.

- **쿠폰 재고는 100개 이며, 101명 에게 발급되면 안 된다. (추후 수치 조절)**
