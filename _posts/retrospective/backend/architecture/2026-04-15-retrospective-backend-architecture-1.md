...
date: 2026-04-15
categories: [Retrospective, Backend, architecture]
...
생각했던것처럼 금방 끝나긴 했다. 그런데 만들면서 뭔가 이상했다. **너무나 익숙하지만, 너무나 안일한 방식이였다.**

진행했던 내 개인 프로젝트처럼 구조를 바꿔보기로 했다. 그러자, **단순 CRUD 하나를 만드는데 매 단계마다 "이게 최선인가?" 라는 질문이 새로운 형태로 돌아왔다.**
...
이런 환경에서 구조를 바꿔보면서 **"공공기관 폐쇄망이 아닌 테크 기업이면 어떻게 짤까"** 와 **"회사 현실에선 어디까지 가능한가"** 사이를 끊임없이 오갔다. 이 글은 그 고민의 기록이다.
...
## 문제 분석
구조 개선 전에 기존 코드 구조를 봤다. 그리고 리뷰어 관점으로 내가 쓰려던 접근을 분석했다.
...
**이건 패키지 분리가 아니라 화면 복제다.** 화면 메뉴는 요구사항으로 수시로 재편된다.(실제로 많이 경험 '당해'봤다.) "개선사항을 별도 최상위 메뉴로 빼주세요" 한 마디에 `import` 경로 전부가 바뀐다.

`Netflix`, `Naver`, `Kakao` 같은 테크 기업 오픈소스를 보면 전부 **도메인 기반 평평한 구조**다. `system.management.xxx` 같은 **화면 메뉴 그룹명은 패키지 구조로 설계하지 않는다.**
...
menu/
  ...
  └─ service/
      ...
      └─ impl/
          ...
          └─ MenuDao.java       (interface)
```

`Service` 인터페이스와 `ServiceImpl` 분리는 **구현체가 하나뿐인데 인터페이스를 두는** `SI` 스러운 습관이다.

`VO` 라는 네이밍은 `MyBatis` 매핑용 `DTO`에 가깝지 진짜 `Value Object` 가 아니다.

`dao` 는 `Repository` 로 불리는 게 **역할과 이름이 매칭된다.**

### 문제 3: 넥사크로의 제약과 특수성
`Nexacro` 는 일반 `REST API` 처럼 `JSON` 을 주고받지 않는다.

- 응답은 반드시 `NexacroResult.addDataset(name, Object)` `NexacroResult.addVariable(name, Object)` 패턴만 가능
- 요청은 자체 `PlatformData` 포맷. `@RequestBody` 로 못 받는다
- 각 `Dataset` 행마다 `RowType` 이 자동 추적된다

`REST + JSON` 마인드로 접근하면 전부 깨진다. 그렇다고 넥사크로를 빼고 생각할 수도 없다. **프레임워크가 요구하는 패턴 안에서 코드를 설계해야 한다.**
...
### 문제 5: 공공기관 + 폐쇄망
**공공기관의 데이터 관리(감사) 관점이 모든 데이터 설계의 최상위 기준**이 된다.
- 공공기록물법, 개인정보보호법
- 감사원/내부 감사 대응: "왜 이 시점에 이 권한이 부여됐는가?"
- 책임 소재 추적

"매핑 테이블은 전체 삭제 후 재저장" 이 표준이라고 알고있지만, 공공기관에서는 **그 단순화가 오히려 리스크**가 된다.

## 설계 방향성

### 원칙
1. **프로젝트 전체를 갈아엎지 않는다.** 본인 영역 안에서만 구조를 개선하고, 팀 일관성은 유지한다.
2. **"아는 것"과 "쓰는 것"을 구분한다.** 더 나은 방향성을 알고도 회사 제약 때문에 선택 못 하는 건 **패배가 아니라 판단이다.**
3. **각 선택의 이유를 말할 수 있어야 한다.** "팀 컨벤션이라서" 는 답이 아니다. **"왜 이 상황에서 이게 맞는지"** 를 설명할 수 있어야 한다.

### 바라는 것 vs 회사 현실

| 항목 | 바라는 것 | 회사 프로젝트에서의 선택                           |
| :--- | :--- |:----------------------------------------|
| 패키지 구조 | 도메인 기반 평평 | `system.management` 유지, 내 영역만 도메인 분리    |
| 계층 구조 | `presentation/application/domain/infrastructure` 4계층 | `controller/service/repository/dto` 3계층 |
| 도메인 모델 | `Entity` 에 비즈니스 로직 풍부하게 | `MyBatis + DTO`                         |
| 날짜 타입 | `LocalDateTime + Jackson` 전역 설정 | `String + TO_CHAR` (log4jdbc 제약)        |
| 응답 포맷 | `ApiResponse<T>` 통일 | `NexacroResult` 강제                      |
| 매핑 테이블 저장 | 전체 삭제 + 재저장 | `Diff 방식` (공공기관 데이터 관리)                 |
...
권한별 메뉴 관리는 `menu` 패키지 안에 둘까, 따로 뺄까? 쿼리가 똑같은데(메뉴 조회 쿼리) 분리하면 중복 아닌가?

**결론: 분리한다.** 주어가 다른 도메인이기 때문이다.
- `menu` 도메인: 메뉴 자체의 `CRUD`
- `menuauth` 도메인: 메뉴 자체와는 다른 **사용자별 메뉴 권한** 매핑 관리

각 업무 화면에서 동일한 쿼리가 발생했다.(메뉴 조회 쿼리) 하지만 그렇다 해도, 분리를 하는 방향으로 진행했다.

앞으로 어떤 요구사항이 추가될지 모른다. **각 도메인이 자기 쿼리를 소유하는 게 결합도 측면에서 이득**이다.

`menuauth` 같은 소문자 연속 패키지명은 어색하다. 그런데 자바 패키지 규칙상 이게 표준이다. **관계를 명시적으로 드러내는 네이밍** 으로 방향성을 잡았다.

### Step 2: 내부 구조는 몇 계층인가
내 개인 프로젝트처럼 하고자 했다. `presentation / application / domain / infrastructure`. 헥사고날 아키텍처도 적용해보고 싶었다.

**현실은 3계층이 맞다.** 이유는 세 가지다.
1. **MyBatis 환경**은 도메인 모델에 비즈니스 로직을 다양하게 담기 어렵다. 어차피 `큰 DTO`가 된다.
...
조회 `Request` 가 비어 있으면? **과감히 삭제한다.** 미리 빈 껍데기를 두는 건 **'이유 없는 존재'** 이기 때문이다.
...
### Step 5: 첫 번째 타협 — `TO_CHAR`
`BaseDto / BaseResponse` 에 날짜 필드를 `LocalDateTime` 으로 받고 `Jackson` 전역 설정으로 포맷팅하려 했다. **그런데 문제가 발생했다.**
...
**3번을 선택했다.** 단일 기능을 개발하면서 팀 전체 인프라를 건드리는 건 **책임 범위를 넘는다.**
...
```java
for (Item item : items) {
    repository.insert(item);  // 수천, 수만 건 이면?
}
```
...
<insert id="insertMenuRoles">
    ...
    <foreach collection="items" item="item">
        INTO TEST (TEST1, TEST2, TEST3, TEST4)
        VALUES (#{item.test1}, #{item.test2}, #{item.test3}, #{item.test4})
    </foreach>
    SELECT 1 FROM DUAL
</insert>
```

`Oracle` 은 `MySQL` 과 달리 다중 `VALUES` 구문이 안 돼서 `INSERT ALL ... SELECT 1 FROM DUAL` 패턴을 쓴다.
...
**단순성만 보면 A, 이력 추적을 보면 B 이다.** 권한별 메뉴 도메인 프로세스 상 단순한 로직이기에, A 를 선택하려고 했다.

그런데 **공공기관 + 폐쇄망** 이라는 조건이 결정적이었다.

- 권한 매핑은 **100% 감사 대상**이다.
- "왜 이 시점에 이 권한이 부여됐는가" 에 답해야 한다.
- 단순 삭제 후 재저장은 **어느 행이 실제로 변경됐는지를 잃는다**

**B를 선택했다.** A 방법이 효율적이라고 해도, 공공기관 환경에서는 B가 맞다. **프로젝트의 요구사항이 기술 선택의 최상위 기준**이 되어야 한다고 생각한다.

### Step 9: 넥사크로의 `RowType`
`Diff` 방식을 사용하기 위해 넥사크로 `Dataset` 의 `RowType` 을 적극적으로 사용했다.

| `RowType` | 의미 |
| :--- | :--- |
| `DataSet.ROW_TYPE_NORMAL` | Normal (변경 없음) |
| `DataSet.ROW_TYPE_INSERTED` | Insert (신규) |
| `DataSet.ROW_TYPE_UPDATED` | Update (수정) |
| `DataSet.ROW_TYPE_DELETED` | Delete (삭제) |
...
public void saveRoles(List<MenuAuthRoleSaveItem> items) {
    ...
    for (MenuAuthRoleSaveItem item : items) {
        if (item.isInsert(item.getRowType()))      toInsert.add(item);
        else if (item.isDelete(item.getRowType())) toDelete.add(item);
    }

    if (!toDelete.isEmpty()) repository.deleteMenuRoles(toDelete, currentUser);
...
public abstract class BaseSaveItem {
    ...
    public boolean isInsert(int rowType) { return DataSet.ROW_TYPE_INSERTED == rowType; }
    public boolean isUpdate(int rowType) { return DataSet.ROW_TYPE_UPDATED == rowType; }
    public boolean isDelete(int rowType) { return DataSet.ROW_TYPE_DELETED == rowType; }
}
```
...
`@RequestBody` 로 받아도 자동적으로 매핑이 될 줄 알았지만, 넥사크로는 `JSON` 이 아니라 자체 `PlatformData` 포맷으로만 전송이 가능했다.
...
`Dataset` 은 본질적으로 `List` 다. 일반 `REST` 에서 필요한 `Request` 래퍼 객체가 **아예 불필요하다.** 만들어둔 `MenuAuthRoleSaveRequest` 를 삭제했다.

### Step 11: @Param
`MyBatis @Param` 관련해서 에러가 발생했다.
...
### 기술적인 것보다 더 중요했던 것
**"아는 것" 과 "쓰는 것" 의 거리를 받아들이기.** 더 나은 방법을 알아도 회사 환경에선 어디까지 적용 가능한지 판단해야 한다. 판단의 결과물이 정석과 달라도 괜찮다. **이유를 말할 수 있으면 된다.**
...
아키텍쳐 정석, 회사 레거시 컨벤션, 폐쇄망 제약, 공공기관 감사 요구 — 이 네 가지가 모두 "좋은 코드" 의 기준에 영향을 준다. 어느 한 축만 보고 판단하면 실패한다.

좋은 아키텍쳐란 **"모든 축을 알고 있는 상태에서 지금 이 환경에 맞는 균형점을 찾는 것"** 이라고 느꼈다. 그건 단순 지식이 아니라 **상황 판단 능력**이다.

레거시 환경은 답답하지만, 동시에 그 답답함 안에서 **의사결정 능력**을 키울 수 있는 곳이기도 하다. 개인 프로젝트에서는 마주칠 수 없는 종류의 제약들이 매일 새로운 문제를 던져준다.
