...
- **설계 개선을 막는다.** patch로 우회 가능하면 개선 가능한 구조를 그대로 두게 된다. 

이 철학은 이 프로젝트의 이원화와 충돌하지 않는다. 오히려 같은 결론이다. 
`Port`는 `Fake` 주입 ->`Java`의 `DI`, `patch`는 `time.sleep`·`yf.Ticker`처럼 **수정할 수 없는 외부 라이브러리에만** 썼다. `Java`에서 허용하는 "어쩔 수 없는 외부 라이브러리" 예외가 이 범위다. 우리 도메인 객체를 patch로 우회한 곳은 없다.
