...
### 대기업 공통 패턴
```
빠른 판정 (Redis) → 즉시 응답 → 무거운 작업 분리 (Kafka)
```
이게 **이벤트 드리븐 아키텍처**의 기본이다.
