...
### application-local.yml - Kafka 설정
```yaml
spring:
  kafka:
...
- `remain >= 0` → 성공
- `remain < 0` → 재고 없음 (이미 재고가 없는 상태(`0`)에서 차감됨)

**3. 재고 부족 시 롤백**
```java
if (remain == null || remain < 0) {
    redisTemplate.opsForValue().increment(stockKey);  // 재고 복구
    redisTemplate.opsForSet().remove(issuedKey, String.valueOf(memberId));  // 발급 명단 제거
    return false;
}
```
...
| **최대 응답시간** | `13,610ms` | `9,590ms` | `7,040ms` | **`162.68ms`** |
...
### 1. Redis 연산 원자성
```
add() → (서버 장애) → DECR 실행 안 됨
             ↑
    발급 명단에만 등록, 재고는 안 줄어듦
```
현재 `add()`와 `DECR`이 별도 명령어라서 중간에 끊기면 데이터 불일치 발생.

### 2. Kafka send 실패
```
Redis 성공 → Kafka send() 실패
                 ↑
    Redis에서는 발급됐는데 DB 에는 없음
```
네트워크 장애 등으로 `Kafka send()`가 실패하면 롤백 처리가 없음.

### 3. Consumer 중복 처리
```
Kafka 메시지 재전송 → 같은 메시지 2번 처리 → DB 중복 저장
```
`Kafka`는 **at-least-once** 방식이라 같은 메시지가 여러 번 올 수 있음.

### 4. Consumer 실패 시 무한 재시도
```
Consumer 처리 실패 → 재시도 → 또 실패 → 재시도 → ...
```
계속 실패하는 메시지가 무한 재시도되면서 다른 메시지 처리를 막음.

### 다음 단계: Phase 5 - 데이터 정합성 강화
