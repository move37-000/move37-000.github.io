...
> @Order(5) 소스는 위 소스와 @Order 순서와 로그 내용만 달라 생략.
...
트랜잭션 advisor를 order 지정 없이 등록하고, 두 aspect와 target 안에서 각각 트랜잭션 활성 여부를 찍었다. `TransactionSynchronizationManager.isActualTransactionActive()`는 "지금 이 스레드에 트랜잭션이 존재하는가" 이다.
